...
//public A<T,Rank>{
	public:
		...
		virtual void f(DataStruct<T, 2> d) const {
		   ...
		   //template<typename U = T, std::enable_if_t<std::is_integral<U>::value>* = nullptr>i
		//template<int R, std::enable_if_t<(Rank>2)>* = nullptr>
		void f(DataStruct<T, Rank> d) const {
			T x = 5;
...
int main(){
	DataStruct<int, 3> d;
	Solver<int, 3> s(std::make_unique<A_CUDA<int, 3>>());
	s.compute_f(d);
}
